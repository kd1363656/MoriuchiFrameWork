...
VSOutputNoLighting main(
	...
	uint4 skinIndex : SKININDEX,    // スキンメッシュのボーンインデックス(何番目のボーンに影響しているか?のデータ(最大4つぶん,スキンメッシュ対応))
	float4 skinWeight : SKINWEIGHT) // ボーンの影響度(そのボーンにどれだけ影響しているのか?のデータ(細田4つぶん、スキンメッシュ対応))
{
	// スキニング---------------->
	if (g_IsSkinMeshObj)
	{
